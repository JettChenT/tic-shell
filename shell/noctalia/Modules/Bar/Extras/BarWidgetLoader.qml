...
Item {
  ...
  readonly property bool isMiniTicIndicator: widgetId === "Workspace" || widgetId === "ActiveWindow"
  ...
  implicitWidth: isMiniTicIndicator && !TicWorkspaceState.collapsed ? 0 : (isVerticalBar ? barHeight : getImplicitSize(loader.item, "implicitWidth"))
  implicitHeight: isMiniTicIndicator && !TicWorkspaceState.collapsed ? 0 : (isVerticalBar ? getImplicitSize(loader.item, "implicitHeight") : barHeight)

  // Remove layout space left by hidden widgets
  visible: (isMiniTicIndicator && !TicWorkspaceState.collapsed) ? false : (loader.item ? ((loader.item.opacity > 0.0) || (loader.item.hasOwnProperty("hideMode") && loader.item.hideMode === "transparent")) : false)
