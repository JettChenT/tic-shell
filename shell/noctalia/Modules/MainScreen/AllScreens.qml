...
Variants {
  ...
  delegate: Item {
    ...
    // Tic workspace rail lives beside Noctalia's own narrow vertical bar.
    Loader {
      active: {
        if (!parent.windowLoaded || !parent.shouldBeActive)
          return false;
        ...
        var position = Settings.getBarPositionForScreen(modelData?.name);
      ...
      sourceComponent: TicWorkspaceWindow {
        screen: modelData
      }

      onLoaded: {
        Logger.d("AllScreens", "TicWorkspaceWindow created for", modelData?.name);
      }
    }
    ...
    // Reserve workspace rail space next to the narrow Noctalia bar.
    Loader {
      active: {
        if (!windowItem.windowLoaded || !windowItem.shouldBeActive)
          return false;

        var position = Settings.getBarPositionForScreen(windowItem.modelData?.name);
        if (position !== "left" && position !== "right")
          return false;
      ...
      sourceComponent: TicWorkspaceExclusionZone {
        screen: windowItem.modelData
      }

      onLoaded: {
        Logger.d("AllScreens", "TicWorkspaceExclusionZone created for", windowItem.modelData?.name);
      }
    }
