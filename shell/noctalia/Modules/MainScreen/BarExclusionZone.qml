...
PanelWindow {
  ...
  readonly property bool isBarEdge: edge === barPosition
  readonly property bool isVerticalBarEdge: isBarEdge && (edge === "left" || edge === "right")
  property real thickness: isBarEdge ? Style.getBarThicknessForScreen(screen?.name) + (isVerticalBarEdge ? TicWorkspaceState.reservedWidth() : 0) : (Settings.data.bar.frameThickness ?? 12)
  ...
  readonly property real barMarginH: (barFloating && edge === barPosition) ? Math.ceil(Settings.data.bar.marginHorizontal) : 0
  readonly property real barMarginV: (barFloating && edge === barPosition) ? Math.ceil(Settings.data.bar.marginVertical) : 0
